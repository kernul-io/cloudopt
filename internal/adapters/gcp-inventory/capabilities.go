...
// IAMLeastPrivilegePolicy returns documented read-only role reference JSON.
...
// LoadCapabilities parses the embedded GCP capability manifest.
func LoadCapabilities() (ports.CapabilityManifest, error) {
	...
	if err := yaml.Unmarshal(capabilitiesYAML, &manifest); err != nil {
		return ports.CapabilityManifest{}, fmt.Errorf("parse gcp capabilities: %w", err)
	}
	if manifest.Provider == "" {
		manifest.Provider = types.ProviderGCP
	}
	return manifest, nil
}
